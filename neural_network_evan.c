...
//    double training_out[] = {0.9};
    ...
    training_in = (double **) malloc(lines * sizeof(double *));
    for (i = 0; i < lines; i++) {
        training_in[i] = (double *) malloc(INPUTS * sizeof(double));
    }
    training_out = (double *) malloc(lines * sizeof(double));
    
	//Get data from file
    for (i = 0; i < lines; i++) {
        for (j = 0; j < INPUTS; j++) {
            fscanf(fp, "%lf", &training_in[i][j]);
            training_in[i][j] = normalize(training_in[i][j]);
        }
        training_out[i] = normalize(training_out[i]);
    }
    //close file
    fclose(fp);
    initialize();
